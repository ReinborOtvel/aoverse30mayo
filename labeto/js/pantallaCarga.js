...
import EntrarGrupo from "./entrarGrupo.js";
...
class Blockchain {
    constructor(main) {
        ...
        this.provedor = new ethers.providers.Web3Provider(window.ethereum);
    ...
    cambiarRed(chainId) {
        ...
        window.ethereum.request({
            ...
            this.main.pagina = new Mensaje(this.main, "No se cambio la red");
        });
    }
    agregarRed(chainId) {
        let hexChainId = window.ethers.utils.hexValue(chainId);
        ...
        window.ethereum.request({
            ...
            this.cambiarRed(chainId);
        }).catch(() => {
            this.main.pagina = new Mensaje(this.main, "No se agrego la red");
        });
    }
    asignarBaseDatos() {
        ...
        if (address == "") {
            this.crearBaseDatos(abi);
        } else {
            this.baseDatos = new ethers.Contract(address, abi, this.signer);
        }
    }
    crearBaseDatos(abi) {
        let creador = "0xa846fb7c4E50a1dbf23c9AEF49e3Fd7B9c7A9343";
        let bytecode = {
            "functionDebugData": {
                "@_13": {
                    "entryPoint": null,
        ...
        let informacion = { lider: {}, grupo: {}, estadisticas: {}, };
        let creadorMayuscula = creador.toUpperCase();
        informacion.lider[creadorMayuscula] = "primero";
        informacion.grupo[creadorMayuscula] = [];
        informacion.estadisticas[creadorMayuscula] = {
            nombre: "MarSprind",
            ataque: 20, magia: 20, salud: 20, inteligencia: 20,
        };
        let JSONInformacion = JSON.stringify(informacion);
        factory.deploy(JSONInformacion).then((contrato) => {
            alert(contrato.address);
        });
    }
    recibirInformacion() {
        ...
        if (informacion == null) {
            ...
            if (estadisticas == undefined) {
                ...
                if (estadisticas == null) {
                    if (this.chainId == 56) {
                        this.main.pagina = new CrearPersonaje(this.main);
                        this.main.pagina.preload();
                    } else {
                        this.agregarRed(56);
                    }
                } else if (this.chainId == 97) {
                    this.main.pagina = new EntrarGrupo(this.main);
                    this.main.pagina.preload();
                } else {
                    this.agregarRed(97);
                }
            } else if (this.chainId == 97) {
                this.main.pagina = new Nivel(this.main);
                this.main.pagina.preload();
            } else {
                this.agregarRed(97);
            }
...
export default class PantallaCarga {
    ...
    preload() {
        ...
        setTimeout(() => {
            this.blockchain = new Blockchain(this.main);
        }, this.ms * 3);
    }
    draw(ms) {
