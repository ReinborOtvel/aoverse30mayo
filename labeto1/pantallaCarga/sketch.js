...
let siguienteImagen = () => {
    ...
    let ultimoIndice = imagenes.length - 1;
...
function metamask() {
    ...
    if (ethereum == undefined) {
        alert("Descargar metamask");
        return;
    }
    let provedor = new window.ethers.providers.Web3Provider(ethereum);
    ethereum.on('accountsChanged', () => location.reload());
    ethereum.on('chainChanged', () => location.reload());
    provedor.send("eth_requestAccounts", []).then(_direcciones => {
        let direccion = _direcciones[0];
        let signer = provedor.getSigner();
        signer.getChainId().then(_chainId => {
            if (_chainId == 97) {
                if (address == "") {
                    crearBaseDatos();
                } else {
                    baseDatos.contrato = new ethers.Contract(baseDatos.direccion, baseDatos.abi, signer);
                    baseDatos.contrato.get().then((text) => {
...
function preload() {
    estructura.preload();
    for (let i = 0; i < 8; i++) {
        let src = `../assets/pantallaCarga${i}.png`;
        imagenes[i] = loadImage(src);
    }
}
function draw() {
    estructura.draw();
    image(imagenes[indice], 0, 0, 640, 360);
}